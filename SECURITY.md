...
- **Information disclosure:** Ensuring redaction works correctly and PII is not leaked in reports or snapshots. Redaction is on by default for both `run` and `snapshot`; the `nvidia-smi` process list is stripped before storage. On DGX Spark and RTX Spark the collectors additionally read `/etc/dgx-release` (whose `DGX_SERIAL_NUMBER` is redacted to `<serial>`; the DMI serial files are not read), ConnectX-7 fabric addresses (redacted as `<lan-ip>`), the `NCCL_*` / `UCX_NET_DEVICES` environment of the NVCheckup process only, and listening inference ports without process names. A serial number, fabric address or process name surviving redaction is in scope.
...
- **Journal trust:** The change journal (`nvcheckup-changes.json` in the user config directory) is user-writable and therefore untrusted. The journal directory and file are created with owner-only permissions (`0700`/`0600`); that guarantee applies to the default per-user directory, so a custom `--journal DIR` must be a directory the user owns. On Linux, `sudo nvcheckup fix` journals under the invoking user's `~/.config/nvcheckup` when `SUDO_USER` is set, otherwise under `/root/.config/nvcheckup`. `undo` validates every entry (expected keys, value shape, allowed registry/file paths) before acting on it and never writes journal contents verbatim to a privileged path or command line. A journal entry that can cause `undo` to write outside the action's own setting is a vulnerability.
