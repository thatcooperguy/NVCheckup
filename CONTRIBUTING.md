...
Scenarios are JSON files under `.github/fieldtest/scenarios/`. A scenario describes one machine: the GPU rows (`index`, `name`, `uuid`, `driver_version`, `pci.bus_id`, the `memory.*`, `temperature.gpu`, `power.*`, `pstate`, `clocks.*`, `fan.speed`, `utilization.gpu`, `pcie.link.*`, `clocks_event_reasons.active`, `compute_cap` fields the shim answers per query field), plus line lists that the other shims replay verbatim (`lspci_lines`, `dpkg_lines`, `lsmod_lines`, `dmesg_lines`, `lscpu_lines`, `cpuinfo_lines`), structured blocks for `dmi`, `meminfo`, `swaps`, `pressure_memory`, `thermal_zones`, `listening_tcp_ports`, `performance_counters_us`, `cx7_ports` (the ConnectX-7 twins that the `ibdev2netdev`, `ibstat` and `ip` shims and the sysfs fixtures are generated from), `other_netdevs`, `systemd_units`, `fwupd_devices`, `fwupd_updates`, `spark_ota_check_summary`, and the file contents the job writes under `NVC_SIM_ROOT` (`dgx_release`, `fastos_release`, `os_release`, `device_tree_model`). The key list in `gb10.json` is authoritative (it is what `scripts/make-simroot.sh` and the shims read); use it for the full shape and `rig3.json` for a plain multi-GPU rig.
